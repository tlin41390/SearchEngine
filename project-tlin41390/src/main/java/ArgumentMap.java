 ...
 *
 * @author CS 272 Software Development (University of SanFrancisco)
 * @version Fall 2021
 */
public class ArgumentMap {
	...
	public void parse(String[] args) {
		// created a for loop that will loop through the string array and create (key,
		// value) pairs.
	...
	public static boolean isFlag(String arg) {
		return arg != null && arg.length() > 1 && arg.codePointAt(0) == 45 && Character.isLetter(arg.charAt(1));
	}
	...
	public boolean hasFlag(String flag) {
		// checks if the map even has the flag or not.
		return this.map.containsKey(flag) != false;
	}
	 ...
	 */
	public boolean hasValue(String flag) { // if statement will get the map with key value flag and check to see if the
		// value is not null.
	...
	public String getString(String flag, String defaultValue) {
		// if statement to check if the value is mapped. If not then return defaultValue
	...
	public Integer getInteger(String flag) {
		/**
		 * try/catch block to see if string can be converted to integer. If so then
		 * return it if not then catch numberformatexception then return null.
		 */
		try {
			return Integer.parseInt(this.map.get(flag));
		} catch (NumberFormatException e) {
